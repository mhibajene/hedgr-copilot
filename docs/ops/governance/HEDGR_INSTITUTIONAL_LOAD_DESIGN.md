...
The forks may be adopted separately. Fork 3's evidence-only trigger path requires a finite envelope and recording mechanism already made operative in §7a. Until then, current amendment procedures continue. If an adopted Fork 3 amendment changes mandatory sources before Fork 2 is operational, existing source-first and separate permanent-main rebind requirements still apply. Reconcile ADR 0026 operational guidance where necessary; any change to its accepted principles follows its superseding-ADR rule.
