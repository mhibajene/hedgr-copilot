...
- **Approved next ticket:** **`UI-ALG-001`** — Stability Engine Allocation Comprehension Pass (**presentation-only** Stability Engine trust-surface refinement; read-only / informational / non-executing). Founder-approved for activation; full execution brief in **§7a**. Scope held to the shipped allocation trust object: `apps/frontend/app/(app)/dashboard/EngineAllocationBands.tsx` and the sibling copy surfaces it composes (`apps/frontend/lib/engine/stability-explainer-copy.ts`, `apps/frontend/lib/engine/protective-guidance-copy.ts`), plus a founder-approved terminology alignment of the posture/notice copy surface (`apps/frontend/lib/engine/posture-context.ts`, `apps/frontend/lib/engine/notices.ts`) so it reads in the same lane vocabulary (Stable balance / Conservative yield / Reserve), and their unit / copy-contract tests. **No** `apps/frontend/lib/engine/types.ts`, **no** `mock.ts`, **no** `useEngineState.ts`, **no** backend, **no** policy, **no** engine logic, **no** allocation-calculation, **no** runtime-state expansion, **no** execution semantics, **no** accounting truth, **no** CI / workflow changes.
...
**Status:** **Active — `UI-ALG-001` (Stability Engine Allocation Comprehension Pass).** Presentation-only Stability Engine trust-surface refinement; read-only / informational / non-executing.

**Objective.** Improve first-time comprehension of how Hedgr positions capital for stability on the shipped dashboard allocation surface, so a user understands within ~5 seconds: most of their balance is held steady, a smaller portion supports stability, a reserve exists for protection, and nothing needs managing — without introducing portfolio-management, optimization, accounting, execution, or autonomous-authority semantics.

**Governing inputs (read in order).** `AGENTS.md`; `docs/ops/HEDGR_STATUS.md` §7 / §7a; `docs/decisions/SPRINT-2-ADR-INDEX.md`; ADRs **0013**, **0014**, **0015**, **0016**; `docs/ops/stability-engine/HEDGR_STABILITY_ENGINE_UI_EXPLORATION_NOTE.md`; `docs/ops/stability-engine/HEDGR_STABILITY_ENGINE_UI_VARIANT_DISPOSITION.md`. Paper ROW 08–11 reviewed as **presentation-only, non-authoritative** evidence (ROW 08 Option B "Dominant + Supporting" and ROW 09 Variant 3 "Primary Surface Edge" inform direction; the underlying comprehension objective was implemented, not the Paper literally — Paper used dollar amounts; the shipped surface stays on informational percentages per ADR 0013).

**Scope (held).**
- `apps/frontend/app/(app)/dashboard/EngineAllocationBands.tsx` — restructured to a dominant **Stable balance** primary holding surface with two quieter supporting lanes (**Conservative yield**, **Reserve**); proportional per-band bars removed; percentages de-emphasized (present, not focal); subtle static **Primary Stability Edge** on the Stable balance lane only (not a chart / bar / progress / axis / badge).
- `apps/frontend/lib/engine/stability-explainer-copy.ts` and `apps/frontend/lib/engine/protective-guidance-copy.ts` — lane vocabulary aligned to keep the in-card trust object internally consistent (the explainer defines the labels shown above it; protective guidance references the Reserve lane). Founder-approved coherent-surface scope.
- `apps/frontend/lib/engine/posture-context.ts` and `apps/frontend/lib/engine/notices.ts` — founder-approved terminology alignment so the posture-context line and notice banners use the same lane vocabulary (liquidity → reserve, core stability → stable balance, growth capacity → conservative yield) and replace the consumer-facing word "defensive" with "protective" per the exploration note. Copy stays informational and non-ledger; notice contract (`informational` + `not your ledger`) preserved.
- Tests: `apps/frontend/__tests__/engine-allocation-bands.test.tsx` assertions updated to the new vocabulary, hierarchy (`data-allocation-lane` primary/supporting), and microcopy; `engine-allocation-bands-panel-copy-contract.test.tsx`, `engine-stability-explainer-copy-contract.test.ts`, `engine-protective-guidance-copy-contract.test.ts`, `engine-posture-context-contract.test.ts`, and `engine-notices-and-mock-contract.test.ts` remain valid (generic non-empty + informational-denylist / non-ledger contracts).

**Out of scope (unchanged).** `apps/frontend/lib/engine/types.ts`, `mock.ts`, `useEngineState.ts`; backend; policy; engine / allocation-calculation logic; posture state machine; runtime-state expansion; navigation / dashboard IA; CI / workflows; ADR status.

**Governance guarantees.** Allocation bands remain **informational targets**, not balances/accounting (ADR 0013); engine remains **read-only / non-executing** with no rebalance/routing/movement semantics (ADR 0014); engine trust expression preserved as system center (ADR 0015); warmth stays doctrine-constrained with no execution theater, numeric scoring, or urgency (ADR 0016). Informational-trust denylist remains enforced across caption, legend, per-lane descriptions, explainer, and guidance.

**Validation (this session).** `pnpm --filter @hedgr/frontend test` — 718 passed; lint (`eslint . --max-warnings=0`) clean; typecheck (`tsc --noEmit`, app + vitest configs) clean.

**Rollback.** Changes are presentation/copy only and fully reversible via single revert of the five source files (`EngineAllocationBands.tsx`, `stability-explainer-copy.ts`, `protective-guidance-copy.ts`, `posture-context.ts`, `notices.ts`) and their test updates; no schema, runtime state, engine contract, or persisted-state changes were introduced. On closeout, record the merged PR and completed-record section, then return §7 / §7a to the no-active-ticket state when no successor is approved.
