...
1. **`docs/ops/HEDGR_RETAIL_UI_ALLOCATION_PATTERN_FAMILY_NOTE.md`** — narrowed A/B/C families; control, challenger, and centerline roles; suggested next use to narrow within Family C first.
2. **`docs/ops/HEDGR_RETAIL_UI_ALLOCATION_VISUAL_PATTERN_GUARDRAILS.md`** — acceptable, risky, and out-of-bounds pattern behavior; core visual principles; working pattern signal for A/B/C.
3. **`docs/ops/HEDGR_RETAIL_UI_ALLOCATION_PATTERN_MICRO_TEST_READOUT.md`** — same-frame outcomes; precision, busyness, and hierarchy judgments for Patterns A/B/C mapped to families.

**Supporting context (broader refinement and critique stack):**

4. **`docs/ops/HEDGR_RETAIL_UI_ALLOCATION_PATTERN_MICRO_TEST.md`** — same-frame protocol; explicit out-of-scope items.
5. **`docs/ops/HEDGR_RETAIL_UI_ALLOCATION_VISUAL_PATTERN_COMPARISON.md`** — pattern-level definitions and comparative questions.
6. **`docs/ops/HEDGR_RETAIL_UI_ALLOCATION_LEGIBILITY_BRIEF.md`** — bounded legibility objectives and deferrals.
7. **`docs/ops/HEDGR_RETAIL_UI_PASS_RESOLUTION_NOTE.md`** — stronger immediate allocation legibility without performance theater (**§6.2**).
8. **`docs/ops/HEDGR_RETAIL_UI_MEANING_REFINEMENT_BRIEF.md`** — allocation as governed meaning, not machinery (**§6.2**).
9. **`docs/ops/HEDGR_RETAIL_UI_PROTOTYPE_CRITIQUE_READOUT.md`** — trust-detail weight and mid-stack congestion precedent.
