...
1. **`docs/ops/retail-ui/allocation/HEDGR_RETAIL_UI_ALLOCATION_LEGIBILITY_BRIEF.md`** — bounded refinement framing for allocation legibility; distribution read, calm weighting, and informational trust.
2. **`docs/ops/retail-ui/status-language/HEDGR_RETAIL_UI_PASS_RESOLUTION_NOTE.md`** — pass resolution; **§6.2** accepts stronger **immediate** allocation legibility within trust-surface discipline (readability, not excitement).
3. **`docs/ops/retail-ui/status-language/HEDGR_RETAIL_UI_MEANING_REFINEMENT_BRIEF.md`** — cross-cutting meaning pass; **§6.2** treats allocation legibility as governed meaning, not machinery.

**Supporting context:**

4. **`docs/ops/retail-ui/prototype/HEDGR_RETAIL_UI_PROTOTYPE_CRITIQUE_READOUT.md`** — C3-R2 prototype readout; trust detail visible but weighting still an open question (“slightly softer than ideal” in cleaner stacks).
5. **`docs/ops/retail-ui/paper/HEDGR_RETAIL_UI_PAPER_REVIEW_READOUT.md`** — static side-by-side critique; reinforces judgment of trust-detail weight and hierarchy under comparison.
6. **`docs/ops/retail-ui/prototype/HEDGR_RETAIL_UI_C3_REFINEMENT_BRIEF.md`** — C3 centerline, order discipline, subordinate placement of Allocation / Trust Detail.
7. **`docs/ops/retail-ui/prototype/HEDGR_RETAIL_UI_C3_REFINEMENT_NOTES.md`** — C3-R1–R4 variables; **trust-detail weight** as an explicit refinement dimension paired with separation and disclosure tone.
8. **`docs/ops/retail-ui/prototype/HEDGR_RETAIL_UI_SECOND_PASS_VARIANT_SCORE_RUN.md`** — second-pass Concept C scores; C3 as refinement baseline; allocation visibility as a scored lens without shadow accounting.
9. **`docs/ops/retail-ui/prototype/HEDGR_RETAIL_UI_CRITIQUE_SCORECARD.md`** — repeatable criteria (e.g. allocation visibility, hierarchy clarity, doctrine fit, calmness, trust legibility).
