...
| **Scope** | `docs/ops/COPILOT_MVP_ADVISORY_LANE_DEFINITION_DRAFT.md`; optional `HEDGR_STATUS.md` cross-link only if governance wants it; `docs/copilot/**` only if **§7a** is explicitly extended. |
...
| **Primary files** | `docs/ops/COPILOT_MVP_ADVISORY_LANE_DEFINITION_DRAFT.md` (or successor canonical name agreed in PR); optional targeted cross-links in `docs/ops/HEDGR_STATUS.md` **only if** governance wants explicit pointer (still doc-only). |
