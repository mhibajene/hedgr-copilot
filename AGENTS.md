...
**Current Lane V route-hygiene activation override:** Internal **D-114** / **§246** activates `CLASS-A-VAL-002-ROUTE-HYGIENE-001` as the sole active nested Lane V refinement; Internal **D-115** / **§247** amends that ticket in place after Founder review to retire the unused `/api/balance` HTTP route and only its endpoint-specific verification. Authority remains limited to the named trust-route and research-query repair, reversible `/` → `/login` redirect, unused `/api/defi` and `/api/balance` retirements, and removal of the historical `/prototype/**` runtime routes plus route-specific stale tests. The shipped `useBalance` hook, pure ledger balance projection, wallet fallback, journey state, arithmetic, and UI behavior remain unchanged. `/dashboard-synthetic-journey` and the legacy `/dashboard?journey=class-a-val-002` compatibility seam remain unchanged. Parent `CLASS-A-VAL-002` remains open and participant distribution remains paused pending live-Form verification. No Form, protocol, evidence, engine, state, fixture, arithmetic, ledger, transaction semantics, backend service, provider, custody, rail, financial capability, Lane E, Lane N, or cross-lane authority follows. `NO CROSS-LANE IMPACT`.
