...
**Synthetic Home balance-copy amendment (2026-09-20; activation pending):** The Founder now authorises one further finite Home UI/copy correction based on the agreed uncarded mockup. §7/§7a controls the exact scope: retain the open dynamic balance and selected-currency estimate, replace the redundant synthetic caption and two-row scope block with “Includes your simulated activity.”, and leave the separate observation card and shipped deposit-first CTA hierarchy intact. The earlier §294–295 permissions remain consumed; this is fresh, narrow authority inside the same sole nested `CLASS-A-VAL-002-WEEKEND-PREP-001` ticket, not a standing refinement or sibling ticket. Commit source before RAP; merge activation to permanent main and verify a separate projection-only RAP rebind before runtime. Distinct runtime and Verifier QA, hosted checks, shipped inspection, technical completion and a further separate rebind apply. No withdrawal-timing relocation, Deposit/Withdraw flow, financial semantics, other route, Form, participant release or cross-lane change follows.
