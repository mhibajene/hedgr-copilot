...
**Founder Bridge MCP identity configuration (2026-09-25):** §315 continues the existing `BRIDGE-MCP-001` item. After this source record merges and a separate permanent-main RAP rebind is verified, the Founder alone may save one exact-email `Founder only` Allow policy and the `HedgrOps Bridge` Cloudflare Access for SaaS OIDC application in the existing Worker-owning Zero Trust organisation. The Cloudflare-side callback is `https://hedgrops-bridge.hedgr.workers.dev/callback`; the owner must confirm the approved sign-in email before the first save and review policy association, login method and actual application session duration before the application save. This permission includes only bounded source/branch-RAP review, ordinary merge, separate main RAP rebind and their automatic Vercel Production deployments. It supersedes §308's Cloudflare policy/application hold only for those two owner actions. Worker identity/subject, variables, secrets, KV/Durable Object resources or bindings, deployment, protected calls and plugin connection remain held. No broader access, paid commitment or cross-lane authority follows. **NO CROSS-LANE IMPACT.**
