---
name: "source-command-closeout"
description: "Migrated source command `closeout`"
---

# source-command-closeout

Use this skill when the user asks to run the migrated source command `closeout`.
...
Purpose:
Close a merged ticket cleanly in repo, without leaving governance drift.

Default order of operations:
1. Repo truth reconciliation
2. ADR / governance-note follow-up only if required

Default stance:
- `docs/ops/HEDGR_STATUS.md` is canonical
- The repository is the sole canonical institutional-memory surface
- Notion is retired and is not a closeout destination
- Obsidian is founder cognition only and is not a closeout destination
...
4. Identify any ADR / governance-note cleanup needed
5. Explicitly state whether closeout is clean or drift remains
...
- Do not create or maintain a parallel knowledge or governance surface
