...
test('external About thesis is directly reachable without changing its research context', async ({ request }) => {
  for (const path of [route, `${route}?review=true`, `${route}?journey=class-a-val-002`]) {
    const response = await request.get(path);
    expect(response.status()).toBe(200);
    const html = await response.text();
    expect(html).toContain('Build financial stability when your currency is unpredictable.');
    expect(html).toContain('does not hold or move real customer money');
  }
});
